...
class WeinsteinBacktest:
    ...
    def print_detailed_signals(self):
        """Imprimir detalle de cada señal con fechas"""
        
        logger.info("\n" + "=" * 60)
        logger.info("DETALLE DE CADA SEÑAL")
        logger.info("=" * 60)
        
        # Ordenar por fecha de entrada
        sorted_results = sorted(self.results, key=lambda x: x['entry_date'])
        
        for idx, result in enumerate(sorted_results, 1):
            logger.info(f"\n[{idx}/{len(sorted_results)}] {result['ticker']} - {result['name']}")
            logger.info(f"  Transición:      {result['stage_transition']}")
            logger.info(f"  Fecha entrada:   {result['entry_date']}")
            logger.info(f"  Precio entrada:  ${result['entry_price']:.2f}")
            
            # Mostrar resultados por horizonte
            if result['horizons']:
                logger.info(f"\n  Resultados por horizonte:")
                for days in [30, 90, 180, 365]:
                    if days in result['horizons']:
                        h = result['horizons'][days]
                        status = "✓" if h['winner'] else "✗"
                        logger.info(f"    {days:3d}d → {h['exit_date']} | ${h['exit_price']:7.2f} | {h['return_pct']:+7.2f}% {status}")
            ...
            if 'current' in result:
                ...
                logger.info(f"\n  Resultado actual:")
                logger.info(f"    Fecha:           {c['date']}")
                logger.info(f"    Precio:          ${c['price']:.2f}")
                logger.info(f"    Retorno:         {c['return_pct']:+.2f}% {status}")
                logger.info(f"    Días en cartera: {c['days_held']}")
...
def main():
    ...
    try:
        ...
        # Imprimir detalle de cada señal
        backtest.print_detailed_signals()
